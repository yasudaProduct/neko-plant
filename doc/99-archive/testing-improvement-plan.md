...
> 主要な施策は実装済みです（CIでのシード自動実行、pgTAPによるDBテスト追加、Playwrightプロジェクトの再編）。
> 計画中の記述（`no-auth` / `authenticated` プロジェクト名など）は現在の構成と異なります。
> **現在のテスト構成は [02-development/testing.md](../02-development/testing.md) を参照してください。**
