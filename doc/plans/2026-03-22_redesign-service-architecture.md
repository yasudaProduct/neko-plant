...
**方針: データ移行は行わない。** v2 はゼロからスタートする。

| テーブル | 方針 | 理由 |
|---------|------|------|
| `evaluations` | **移行しない・削除** | v2 の posts とは概念が異なる。既存データは捨てる |
| `evaluation_reactions` | **移行しない・削除** | 同上 |
| `plant_images` | **移行しない・削除** | post_images に置き換え。既存画像は参照不能になるが許容 |
| `plant_favorites` | **移行しない・削除** | 同上 |
| `plant_have` | **移行しない・削除** | 同上 |
| `neko` | **維持** | 猫種マスタは移行不要（そのまま利用） |

Prismaマイグレーションでは旧テーブル・enumの DROP のみ実行する。INSERT...SELECT は不要。
...
export type Plant = {
  id: number;
  name: string;
  mainImageUrl?: string;         // post_images の最新画像から取得。投稿がない植物は undefined
...
**`mainImageUrl` の取得クエリ例:**
```sql
-- plants に紐づく最新の post_image を取得
SELECT pi.image_url
FROM post_images pi
...
`plant_images` テーブルは廃止。投稿のない植物は `mainImageUrl` が `undefined` になる（植物カード上は「No image」表示）。
...
| バケット | 変更 | 内容 |
|---------|------|------|
| `plants` | **廃止** | plant_images 用。削除してよい（既存画像は参照不能になるが許容） |
| `evaluations` | **廃止** | データ移行なし。削除してよい（既存画像は参照不能になるが許容） |
| `posts` | **新規作成** | 投稿画像用の統一バケット。v2 の新規投稿から使用 |
| `user_profiles` | 変更なし | — |
| `user_pets` | 変更なし | — |
...
| # | タスク | 詳細 |
|---|--------|------|
| 1 | Prismaスキーマ変更 | posts, post_images, post_likes 追加。旧テーブル・enum 削除（DROPのみ、データ移行なし） |
| 2 | マイグレーション実行 | `npx prisma migrate dev`。データ移行SQL不要 |
| 3 | 型定義の更新 | `types/post.ts` 新規作成、`types/plant.ts` 修正（mainImageUrl取得元変更）、`types/evaluation.ts` 削除 |
| 4 | `post-action.ts` 作成 | `evaluation-action.ts` を置き換え。CRUD + いいね |
| 5 | `plant-action.ts` 修正 | favorite/have/評価集計を削除、共存実績集計・mainImageUrl取得ロジックを追加 |
| 6 | `user-action.ts` 修正 | evaluation/favorite/have 関連を削除、post 関連を追加 |
| 7 | Storage バケット対応 | Supabase で `posts` バケット新規作成（public）。`plants`/`evaluations` バケット廃止 |
| 8 | RLSポリシー設定 | `posts`, `post_images`, `post_likes` テーブルに RLS ポリシー追加（マイグレーションSQL内） |
| 9 | ホームページ変更 | フィード型UIに変更 |
| 10 | 投稿フロー変更 | `/posts/new` をフォトSNS投稿フローに修正（good/bad削除、猫選択追加） |
| 11 | 植物詳細ページ変更 | 共存実績表示 + 関連投稿ギャラリー |
| 12 | 植物一覧ページ変更 | 共存実績ベースの表示・ソート |
| 13 | ユーザープロフィール変更 | 投稿ギャラリー化 |
| 14 | 管理者ページ変更 | `/admin/evaluations` → `/admin/posts` に変更。投稿モデレーション対応 |
| 15 | コンポーネント更新 | 評価関連コンポーネントを投稿対応に書き換え（PlantCard.tsxのgoodCount/badCount等） |
| 16 | 旧ファイル削除 | `evaluation-action.ts`, `types/evaluation.ts`, 旧評価コンポーネント群 |
| 17 | テスト更新 | ユニットテスト + E2Eテストの修正 |
| 18 | ビルド確認 | `npm run build` の成功を確認 |
...
| リスク | 対策 |
|--------|------|
| import先変更の漏れ（evaluation → post） | `npm run build` で型エラーとして検出。grepで全箇所確認してから削除 |
| evaluation-action を参照するコンポーネントの修正漏れ | ビルドエラーで検出。タスク15・16は必ずビルド後に削除 |
| RLSポリシーの未設定 | タスク8でマイグレーションSQL内に記述。既存テーブルのRLS設定を参考に |
| mainImageUrl が undefined の植物が増える | 投稿がない植物は画像なしになる。PlantCard で No image を正しく表示できているか確認 |
| posts バケットのRLSポリシー | Supabase Storage の RLS も設定が必要（認証ユーザーのみアップロード可等） |
