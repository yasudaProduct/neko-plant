...
CREATE POLICY "認証済みユーザーのみが植物画像をアップロード可能" 
ON storage.objects
FOR INSERT
TO authenticated
WITH CHECK (
...
CREATE POLICY "認証済みユーザーのみが自分がアップロードした植物画像を更新可能" 
ON storage.objects
FOR UPDATE
TO authenticated
USING (
...
CREATE POLICY "認証済みユーザーのみが自分がアップロードした植物画像を削除可能" 
ON storage.objects
FOR DELETE
TO authenticated
USING (
